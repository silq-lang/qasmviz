...
def collect_costs(circuit, *, clifford_t: bool, cx_u: bool, cx_sx: bool, ecr_sx: bool, cz_sx: bool, iswap_rx: bool, rzz_rx: bool, rxx_rx: bool, cx_rx: bool, cz_rx: bool, xxphase_rx: bool, quantinuum_h: bool, aqt: bool, tk2: bool, ionq_aria: bool, ionq_forte: bool, syc_phxz: bool, sqrtiswap_phxz: bool, quera_gemini: bool, fez: bool, ibm_eagle: bool, ibm_heron: bool, ibm_heron_frac: bool, rigetti: bool, iqm: bool) -> dict:
    ...
    virtual_rz = cx_sx or ecr_sx or cz_sx or iswap_rx or rzz_rx or rxx_rx or cx_rx or cz_rx or xxphase_rx or quantinuum_h or aqt or iqm or ionq_aria or ionq_forte or syc_phxz or sqrtiswap_phxz or quera_gemini or fez or ibm_eagle or ibm_heron or ibm_heron_frac or rigetti
    primitive_1q = "sx" if (cx_sx or ecr_sx or cz_sx or ibm_eagle or ibm_heron) else "rx" if (iswap_rx or rzz_rx or rxx_rx or cx_rx or cz_rx or xxphase_rx or rigetti or ibm_heron_frac) else "phxz" if (syc_phxz or sqrtiswap_phxz or quera_gemini) else "phasedx" if quantinuum_h else "prx" if iqm else "r" if aqt else "u3" if tk2 else "gpi2" if (ionq_aria or ionq_forte) else None
    ...
    if clifford_t or cx_u or cx_sx:
        ...
        depth, count = metric_depth_and_count(
            circuit,
            is_interesting=lambda node: node.op.name == "tk2",
            respect_barriers=True,
        )
        if count:
            data["tk2-count"] = count
            data["tk2-depth"] = depth
    elif xxphase_rx:
        depth, count = metric_depth_and_count(
            circuit,
    ...
    if primitive_1q is not None:
        if primitive_1q == "gpi2":
            ...
            # AQT: R gate has no dagger variant; just match "r".
            p1_depth, p1_count = metric_depth_and_count(
                circuit,
                is_interesting=lambda node: node.op.name == "r",
                respect_barriers=True,
            )
        elif primitive_1q == "u3":
            # TK2: u3 has no dagger variant; just match "u3".
            p1_depth, p1_count = metric_depth_and_count(
                circuit,
                is_interesting=lambda node: node.op.name == "u3",
                respect_barriers=True,
            )
        else:
            p1_depth, p1_count = metric_depth_and_count(
...
_PYTKET_GATE_DEFS = {
    ...
    # R(angle_t, phase_t) — AQT native single-qubit gate, same as PhasedX
    "r": (
        "gate r(angle_t, phase_t) q0 {\n"
        "  rz(-phase_t*pi) q0;\n"
        "  rx(angle_t*pi) q0;\n"
        "  rz(phase_t*pi) q0;\n"
        "}"
    ),
    # rxx(theta) = exp(-i*theta/2 * XX) — AQT native two-qubit gate
    "rxx": (
        "gate rxx(theta) q0,q1 {\n"
        "  cx q0,q1;\n"
        "  rx(theta) q0;\n"
        "  cx q0,q1;\n"
        "}"
...
def _pytket_op_to_qasm_line(cmd, qubit_to_name: dict, precision: int = 10, phasedx_gate_name: str = "phasedx", xxphase_gate_name: str = "xxphase") -> str | None:
    """
    ...
    (e.g. ``"prx"`` for IQM, ``"r"`` for AQT).
    ``xxphase_gate_name`` controls the QASM gate name used for OpType.XXPhase
    (e.g. ``"rxx"`` for AQT, which is already in qelib1 and needs no gate def).
    """
    import math as _math
    ...
    if optype == OpType.XXPhase:
        if xxphase_gate_name == "rxx":
            # rxx is in qelib1 with angle in radians
            return f"rxx({fmt(ht_to_rad(params[0]))}) {qs[0]}, {qs[1]};"
        return f"{xxphase_gate_name}({fmt(params[0])}) {qs[0]}, {qs[1]};"
...
def _compile_pytket(qc, *, rebase_pass, gateset_name: str, phasedx_gate_name: str = "phasedx", xxphase_gate_name: str = "xxphase"):
    """
    ...
    (default ``"phasedx"``; use ``"prx"`` for IQM, ``"r"`` for AQT).

    ``xxphase_gate_name`` is the QASM gate name to use for OpType.XXPhase
    (default ``"xxphase"``; use ``"rxx"`` for AQT, which is already in qelib1).
    ...
    for cmd in tk_circuit.get_commands():
        if cmd.op.type in _PHASEDX_TYPES:
            ...
            needed_defs.add(xxphase_gate_name)
    ...
    for cmd in tk_circuit.get_commands():
        line = _pytket_op_to_qasm_line(cmd, qubit_to_name, phasedx_gate_name=phasedx_gate_name, xxphase_gate_name=xxphase_gate_name)
        if line is not None:
            lines.append(line)
    ...
    # --- Step 4: QASM2 → Qiskit ---
    from qiskit.qasm2 import CustomInstruction as _CI
    from qiskit.circuit import Gate as _Gate, QuantumCircuit as _QC
    import numpy as _np

    _custom = []

    if phasedx_gate_name in ("phasedx", "prx", "r"):
        _name = phasedx_gate_name
        class _PhasedXGate(_Gate):
            _gname = _name
            def __init__(self, a, b):
                super().__init__(self._gname, 1, [a, b])
            def _define(self):
                qc = _QC(1); a, b = float(self.params[0]), float(self.params[1])
                qc.rz(-b * _np.pi, 0); qc.rx(a * _np.pi, 0); qc.rz(b * _np.pi, 0)
                self.definition = qc
        _custom.append(_CI(_name, 2, 1, _PhasedXGate))

    if xxphase_gate_name == "rxx":
        pass  # rxx is defined via gate def in the QASM output

    qiskit_circuit = qiskit_qasm2.loads(qasm2_out, custom_instructions=_custom if _custom else None)
...
def _compile_pytket_tk2(qc):
    """
    Compile a Qiskit QuantumCircuit to the pytket TK2+TK1 gateset, returning
    (pytket_circuit, qiskit_circuit, qasm2_out).

    TK2(a,b,c) = XXPhase(a) YYPhase(b) ZZPhase(c) is pytket's canonical
    hardware-agnostic 2Q gate (the KAK / Weyl-chamber decomposition).
    TK1(a,b,c) = Rz(a) Rx(b) Rz(c) is the companion 1Q gate.

    pytket's own circuit_to_qasm_str is used for QASM emission; it emits a
    ``tk2`` gate definition automatically, so no custom emission is needed.

    Raises SystemExit if pytket is not installed.
    """
    try:
        from pytket import OpType
        from pytket.passes import KAKDecomposition, AutoRebase, SequencePass
        from pytket.qasm import circuit_from_qasm_str, circuit_to_qasm_str
    except ImportError:
        raise SystemExit("--tk2 requires pytket: pip install pytket")

    from qiskit import qasm2 as qiskit_qasm2

    # Qiskit → QASM 2 → pytket
    qasm2_str = qiskit_qasm2.dumps(qc)
    qasm2_str = re.sub(r'\bp\(', 'rz(', qasm2_str)
    tk_circuit = circuit_from_qasm_str(qasm2_str)

    # Compile: KAK decomposition to TK2, then rebase 1Q gates to TK1
    import warnings as _warnings
    with _warnings.catch_warnings():
        _warnings.filterwarnings("ignore", category=DeprecationWarning)
        SequencePass([
            KAKDecomposition(target_2qb_gate=OpType.TK2),
            AutoRebase({OpType.TK2, OpType.TK1}),
        ]).apply(tk_circuit)

    # Emit via pytket's own QASM emitter (generates tk2 gate def automatically)
    qasm2_out = circuit_to_qasm_str(tk_circuit)

    # Re-parse in Qiskit (no custom instructions needed; tk2 def is in the QASM)
...
def main() -> None:
    ...
    target_portable.add_argument(
        ...
        action="store_true",
        dest="tk2",
        help="transpile into the pytket TK2+TK1 gateset. Requires `pytket`.",
    )
    target_portable.add_argument(
        "--clifford-t",
        action="store_true",
    ...
    target_vendor.add_argument(
        ...
        action="store_true",
        dest="iqm",
        help="transpile into the IQM native basis {prx, cz}. Requires `pytket`.",
    )
    target_vendor.add_argument(
        "--aqt",
        action="store_true",
        dest="aqt",
        help="transpile into the AQT native basis {r, rxx}. Requires `pytket`.",
    )
    target_vendor.add_argument(
        "--quantinuum-h",
        action="store_true",
    ...
    _targets = [name for name in (
        ...
        "iqm", "aqt", "quantinuum_h", "google_sycamore", "google_sqrtiswap", "quera_gemini",
    ...
    basis_kwargs = dict(clifford_t=args.clifford_t, cx_u=args.cx_u, tk2=args.tk2, cx_sx=args.cx_sx, ecr_sx=args.ecr_sx, cz_sx=args.cz_sx, iswap_rx=args.iswap_rx, rzz_rx=args.rzz_rx, rxx_rx=args.rxx_rx, cx_rx=args.cx_rx, cz_rx=args.cz_rx, xxphase_rx=args.xxphase_rx, quantinuum_h=args.quantinuum_h, aqt=args.aqt, ionq_aria=args.ionq_aria, ionq_forte=args.ionq_forte, syc_phxz=args.google_sycamore, sqrtiswap_phxz=args.sqrtiswap_phxz or args.google_sqrtiswap, quera_gemini=args.quera_gemini, fez=args.fez, ibm_eagle=args.ibm_eagle, ibm_heron=args.ibm_heron, ibm_heron_frac=args.ibm_heron_frac, rigetti=args.rigetti, iqm=args.iqm)
    ...
    for file_idx, (filename, qasm3_code) in enumerate(inputs):
        ...
        if args.fez:
            ...
            _tk_circuit, selected, _pytket_qasm2 = _compile_pytket_tk2(qc)
            ...
            try:
                ...
                raise SystemExit("--iqm requires pytket: pip install pytket")
            rebase = AutoRebase({OpType.PhasedX, OpType.CZ, OpType.Rz})
            _tk_circuit, selected, _pytket_qasm2 = _compile_pytket(
                qc, rebase_pass=rebase, gateset_name="iqm",
            ...
            try:
                ...
                raise SystemExit("--aqt requires pytket: pip install pytket")
            rebase = AutoRebase({OpType.PhasedX, OpType.XXPhase, OpType.Rz})
            _tk_circuit, selected, _pytket_qasm2 = _compile_pytket(
                qc, rebase_pass=rebase, gateset_name="aqt",
                phasedx_gate_name="r",
        ...
        if do_show:
            ...
            if _cirq_qasm2 is not None or _pytket_qasm2 is not None:
                for name in ("Phxz", "Syc", "Sqrt_iswap", "Gpi2", "Gpi", "Ms", "Zz", "Phasedx", "Zzphase", "Zzmax", "Xxphase", "Prx", "Rxx", "R(", "Tk2", "U3"):
